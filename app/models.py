...
class User(UserMixin,db.Model):
    ...
    blogger = db.relationship('blog',backref = 'user', lazy = 'dynamic')
...
class blog(db.Model):
    ...
    @classmethod
    def count_blogger(cls,uname):
        user = User.query.filter_by(username = uname).first()
        blogger = blog.query.filter_by(user_id = user.id).all()
...
class Comment(db.Model):
    ...
    user_id = db.Column(db.Integer,db.ForeignKey("users.id"))
    blog = db.Column(db.Integer,db.ForeignKey("blogger.id"))
